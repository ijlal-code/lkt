        ...
        <div class="card-body p-5" style="min-height: 800px; font-family: 'Times New Roman', serif; position: relative;">
            ...
            <div class="row mt-5">
                <div class="col-6 offset-6 text-center">
                    <p class="mb-5">Pangkep, {{ $sp2a->tanggal_surat->format('d F Y') }}<br>Hormat Kami,</p>
                    
                    <div style="height: 100px; display: flex; align-items: center; justify-content: center;">
                        @if($sp2a->current_step == 'finished')
                            <div class="border border-success text-success p-2 rounded fw-bold opacity-75" style="transform: rotate(-5deg);">
                                APPROVED BY SYSTEM<br>
                                <small style="font-size: 10px;">{{ $sp2a->approved_gm_at }}</small>
                            </div>
                        @else
                            <span class="text-muted fst-italic bg-light p-2 rounded">[Menunggu Tanda Tangan GM]</span>
                        @endif
                    </div>

                    <p class="fw-bold mt-3 text-decoration-underline">{{ $sp2a->penanda_tangan_nama }}</p>
                    <p>GM Internal Audit</p>
                </div>
            </div>

             @if($sp2a->email_auditor || $sp2a->email_k3)
             <div class="mt-5 small text-muted">
                 <strong>Tembusan:</strong>
                 <ul class="mb-0">
                     @if($sp2a->email_auditor) <li>Auditor</li> @endif
                     @if($sp2a->email_k3) <li>Unit K3</li> @endif
                     @if($sp2a->email_atasan) <li>Atasan Unit</li> @endif
                 </ul>
             </div>
             @endif
        </div>
    ...
    <div class="card fixed-bottom shadow-lg border-top border-primary">
        <div class="card-body container d-flex justify-content-between align-items-center py-3">
            <div>
                ...
                <small>Anda login sebagai <strong>{{ strtoupper(Auth::user()->role) }}</strong>. Silakan periksa dokumen.</small>
            </div>
                ...
                <form action="{{ route('sp2a.koreksi', $sp2a->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <textarea name="catatan" class="form-control" rows="4" required placeholder="Tuliskan apa yang salah dan perlu diperbaiki oleh staff..."></textarea>
                        </div>
                    </div>
